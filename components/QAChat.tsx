...
const QAChat: React.FC<QAChatProps> = ({ initialLanguage, analysisData }) => {
  ...
  const containerClasses = isExpanded
    ? "fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in"
    : "flex flex-col h-[600px] bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden animate-fade-in";

  const contentClasses = isExpanded
    ? "bg-white w-full max-w-4xl h-[85vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden relative"
    : "flex flex-col h-full w-full";

  return (
    <div className={containerClasses} style={{ animationDelay: isExpanded ? '0s' : '0.3s' }}>
      
      {/* Background click to close (only in expanded mode) */}
      {isExpanded && <div className="absolute inset-0" onClick={toggleExpand}></div>}
